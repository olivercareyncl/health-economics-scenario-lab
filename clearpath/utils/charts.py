...
def make_scenario_outcome_chart(scenario_df):
    ...
    fig.add_trace(
        go.Bar(
            x=scenario_df["Scenario"],
            y=scenario_df["Cases shifted earlier"],
            name="Cases shifted earlier",
            text=[f"{v:,.0f}" for v in scenario_df["Cases shifted earlier"]],
            textposition="outside",
        )
    )
    ...
    fig.add_trace(
        go.Bar(
            x=scenario_df["Scenario"],
            y=scenario_df["Emergency presentations avoided"],
            name="Emergency presentations avoided",
            text=[f"{v:,.0f}" for v in scenario_df["Emergency presentations avoided"]],
            textposition="outside",
        )
    )

    fig.update_layout(
        title="Headline pathway impact across scenarios",
        barmode="group",
        xaxis_title="",
        yaxis_title="Estimated volume",
        height=450,
...
def make_cases_shifted_chart(yearly_df):
    fig = go.Figure(
        data=[
            go.Bar(
                x=yearly_df["year"],
                y=yearly_df["cases_shifted_earlier"],
                text=[f"{v:,.0f}" for v in yearly_df["cases_shifted_earlier"]],
                textposition="outside",
                showlegend=False,
            )
        ]
    ...
    fig.update_layout(
        title="Cases shifted earlier by year",
        xaxis_title="Year",
        yaxis_title="Cases shifted",
        height=430,
        margin=dict(l=20, r=20, t=80, b=20),
        showlegend=False,
    )
...
def make_comparator_delta_chart(base_results: dict, comparator_results: dict, comparator_label: str):
    ...
    values = [
        ...
        comparator_results["emergency_presentations_avoided_total"] - base_results["emergency_presentations_avoided_total"],
