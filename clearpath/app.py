...
def load_defaults() -> dict:
    ...
    fallback_defaults = {
        ...
        "admissions_per_emergency_presentation": 1.0,
        "average_length_of_stay": 6.0,
        "intervention_cost_per_case_reached": 400.0,
        "treatment_cost_early": 12000.0,
        "treatment_cost_late": 20000.0,
        "cost_per_emergency_admission": 4000.0,
        "cost_per_bed_day": 450.0,
...
def build_yearly_results_table(yearly_df: pd.DataFrame) -> pd.DataFrame:
    ...
    display_df = display_df.rename(
        columns={
            "year": "Year",
            "cases_reached": "Cases reached",
            "cases_shifted_earlier": "Cases shifted earlier",
            "emergency_presentations_avoided": "Emergency presentations avoided",
    ...
    number_cols = [
        ...
        "Emergency presentations avoided",
        "Admissions avoided",
        "Bed days avoided",
    ]
...
def build_uncertainty_table(uncertainty_df: pd.DataFrame) -> pd.DataFrame:
    ...
    display_df = display_df.rename(
        columns={
            "case": "Case",
            "cases_shifted_total": "Cases shifted earlier",
    ...
    display_df["Cases shifted earlier"] = display_df["Cases shifted earlier"].apply(format_number)
        ...
        rows.append(
            {
                ...
                "Cases shifted earlier": scenario_results["cases_shifted_total"],
                "Emergency presentations avoided": scenario_results["emergency_presentations_avoided_total"],
...
def format_scenario_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in ["Cases shifted earlier", "Emergency presentations avoided"]:
...
def get_best_scenario_text(df: pd.DataFrame) -> dict:
    ...
    best_health_gain = df.loc[df["Cases shifted earlier"].idxmax()]
...
def build_comparator_table(base_results: dict, comparator_results: dict) -> pd.DataFrame:
    rows = [
        {
            "Metric": "Cases shifted earlier",
            "Current selection": format_number(base_results["cases_shifted_total"]),
            "Comparator": format_number(comparator_results["cases_shifted_total"]),
            "Delta": format_number(comparator_results["cases_shifted_total"] - base_results["cases_shifted_total"]),
        },
        {
            "Metric": "Emergency presentations avoided",
            "Current selection": format_number(base_results["emergency_presentations_avoided_total"]),
            "Comparator": format_number(comparator_results["emergency_presentations_avoided_total"]),
            "Delta": format_number(
                comparator_results["emergency_presentations_avoided_total"]
                - base_results["emergency_presentations_avoided_total"]
            ),
        },
        {
...
st.title("ClearPath")
st.subheader("Early Diagnosis Value Sandbox")
st.write(
    "An interactive sandbox for testing how earlier diagnosis changes cost, activity, and value under different assumptions."
)
...
with st.sidebar:
    ...
    st.header("Population and diagnosis")
    annual_incident_cases = st.number_input(
        "Annual incident cases",
        min_value=0,
        value=int(scenario_inputs.get("annual_incident_cases", defaults["annual_incident_cases"])),
        step=50,
    )
    current_late_diagnosis_rate = st.slider(
        "Current late diagnosis rate",
        min_value=0.0,
        max_value=1.0,
        value=float(scenario_inputs.get("current_late_diagnosis_rate", defaults["current_late_diagnosis_rate"])),
        step=0.01,
    )
    achievable_reduction_in_late_diagnosis = st.slider(
        "Achievable reduction in late diagnosis",
        min_value=0.0,
        max_value=0.5,
        value=float(
            scenario_inputs.get(
                "achievable_reduction_in_late_diagnosis",
                defaults["achievable_reduction_in_late_diagnosis"],
            )
        ),
        step=0.01,
    )
    ...
    late_emergency_presentation_rate = st.slider(
        "Emergency presentation rate, later diagnosis",
        min_value=0.0,
        max_value=1.0,
        value=float(
            scenario_inputs.get("late_emergency_presentation_rate", defaults["late_emergency_presentation_rate"])
        ),
        step=0.01,
    )
    early_emergency_presentation_rate = st.slider(
        "Emergency presentation rate, earlier diagnosis",
        min_value=0.0,
        max_value=1.0,
        value=float(
            scenario_inputs.get("early_emergency_presentation_rate", defaults["early_emergency_presentation_rate"])
        ),
        step=0.01,
    )
    admissions_per_emergency_presentation = st.number_input(
        "Admissions per emergency presentation",
        min_value=0.0,
        value=float(
            scenario_inputs.get(
                "admissions_per_emergency_presentation",
                defaults["admissions_per_emergency_presentation"],
            )
        ),
        step=0.1,
    )
    ...
    intervention_cost_per_case_reached = st.number_input(
        "Intervention cost per case reached",
        min_value=0.0,
        value=float(
            scenario_inputs.get(
                "intervention_cost_per_case_reached",
                defaults["intervention_cost_per_case_reached"],
            )
        ),
        step=25.0,
    )
    treatment_cost_early = st.number_input(
        "Treatment cost per earlier diagnosis case",
        min_value=0.0,
        value=float(scenario_inputs.get("treatment_cost_early", defaults["treatment_cost_early"])),
        step=500.0,
    )
    treatment_cost_late = st.number_input(
        "Treatment cost per later diagnosis case",
        min_value=0.0,
        value=float(scenario_inputs.get("treatment_cost_late", defaults["treatment_cost_late"])),
        step=500.0,
    )
    cost_per_emergency_admission = st.number_input(
        "Cost per emergency admission",
        min_value=0.0,
        value=float(
            scenario_inputs.get("cost_per_emergency_admission", defaults["cost_per_emergency_admission"])
        ),
        step=100.0,
    )
    ...
    qaly_gain_per_case_shifted = st.number_input(
        "QALY gain per case shifted earlier",
        min_value=0.0,
        value=float(
            scenario_inputs.get("qaly_gain_per_case_shifted", defaults["qaly_gain_per_case_shifted"])
        ),
        step=0.01,
    )
...
inputs = {
    "annual_incident_cases": annual_incident_cases,
    "current_late_diagnosis_rate": current_late_diagnosis_rate,
    "achievable_reduction_in_late_diagnosis": achievable_reduction_in_late_diagnosis,
    "late_emergency_presentation_rate": late_emergency_presentation_rate,
    "early_emergency_presentation_rate": early_emergency_presentation_rate,
    "admissions_per_emergency_presentation": admissions_per_emergency_presentation,
    "average_length_of_stay": average_length_of_stay,
    "intervention_cost_per_case_reached": intervention_cost_per_case_reached,
    "treatment_cost_early": treatment_cost_early,
    "treatment_cost_late": treatment_cost_late,
    "cost_per_emergency_admission": cost_per_emergency_admission,
    "cost_per_bed_day": cost_per_bed_day,
    "costing_method": costing_method,
    "qaly_gain_per_case_shifted": qaly_gain_per_case_shifted,
...
with tab1:
    ...
    col1.metric("Cases shifted earlier", format_number(results["cases_shifted_total"]))
    col2.metric(
        "Emergency presentations avoided",
        format_number(results["emergency_presentations_avoided_total"]),
    )
    ...
    col10.metric("Max intervention cost per case", format_currency(results["break_even_cost_per_case"]))
    col11.metric(
        "Required late diagnosis reduction",
        format_percent(results["break_even_reduction_in_late_diagnosis"]),
    )
    ...
    if inputs["costing_method"] == "Combined illustrative view":
        st.caption(
            "Combined illustrative view adds treatment cost difference and emergency/admission savings together. This is useful for exploration, but may overstate value if local costing assumptions overlap."
        )
    ...
    st.plotly_chart(make_cases_shifted_chart(results["yearly_results"]), use_container_width=True)
    ...
    comp_col1.metric(
        "Cases shifted delta",
        format_number(comparator_results["cases_shifted_total"] - results["cases_shifted_total"]),
    )
    comp_col2.metric(
    ...
    threshold_col1.metric(
        "Max intervention cost per case",
        format_currency(results["break_even_cost_per_case"]),
    )
...
with tab4:
    ...
    st.write(
        "These preset scenarios illustrate how different targeting and delivery choices change the case for earlier diagnosis over the selected horizon."
    )
    ...
    with summary_col3:
        st.metric(
            ...
            format_number(best_scenarios["best_health_gain"]["Cases shifted earlier"]) + " cases shifted",
        )
...
st.markdown("---")
st.caption(
    "ClearPath is part of the Health Economics Scenario Lab — a series of interactive decision sandboxes for exploring value under uncertainty."
)