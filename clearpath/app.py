    ...
    rows = [
        {
            ...
            "Delta": format_number(
                comparator_results["cases_shifted_total"] - base_results["cases_shifted_total"]
            ),
        },
        {
...
st.title("ClearPath")
st.subheader("Early Diagnosis Sandbox")
st.write(
    "Explore how earlier diagnosis might change pathway pressure, emergency activity, admissions, and value under different assumptions about reach, effect size, and delivery cost."
)

st.info(
    "Key question: What would need to shift for earlier diagnosis to create value?"
)
...
with tab1:
    ...
    primary_1.metric("Cases shifted earlier", format_number(results["cases_shifted_total"]))
    primary_2.metric(
        "Emergency presentations avoided",
    ...
    st.plotly_chart(
        make_cases_shifted_chart(results["yearly_results"]),
