...
function DesktopDecisionRail({
  ...
  netCostValue: string;
  costPerQaly: string;
  stabilisedPatients: string;
  crisisEventsAvoided: string;
  mainDriver: string;
  interpretation: {
    ...
    <div className="sticky top-6 space-y-4">
      <div className={PANEL_SHELL}>
        ...
        <div className="mt-4 grid grid-cols-2 gap-3">
          <MetricCard label="Patients stabilised" value={stabilisedPatients} />
          <MetricCard label="Crisis events avoided" value={crisisEventsAvoided} />
        </div>
      </div>
    ...
    <div className="mx-auto max-w-7xl px-4 py-5 sm:px-6 lg:px-8 lg:py-8">
      ...
      <div className="hidden lg:grid lg:grid-cols-[minmax(0,1.18fr)_392px] lg:gap-6 xl:grid-cols-[minmax(0,1.24fr)_408px]">
        <main className="min-w-0 space-y-5">
          ...
          <SectionCard
            title="Charts"
            description="Use the first row for trajectory and economics. Use the second row for pathway impact and bounded sensitivity."
            dense
          >
            <div className="grid gap-4 xl:grid-cols-2">
              <StabilisedPatientsChart yearlyResults={results.yearly_results} />
        ...
        <aside className="min-w-0">
          <DesktopDecisionRail
            ...
            stabilisedPatients={formatNumber(results.patients_stabilised_total)}
            crisisEventsAvoided={formatNumber(results.crisis_events_avoided_total)}
