import type { ComparatorOption, CostingMethod, Inputs, TargetingMode } from "@/lib/stableheart/types";
...
export const TARGETING_MODE_MAP: Record<TargetingMode, { population_multiplier: number; reach_multiplier: number; risk_multiplier: number }> = {
  "Broad cardiovascular risk cohort": {
...
export const COSTING_METHOD_MAP: Record<CostingMethod, { mode: "event_admission" | "bed_day" | "combined" }> = {
...
function getStrongerRiskReduction(defaults: Inputs): Partial<Inputs> {
  return {
    ...defaults,
    risk_reduction_in_recurrent_events: defaults.risk_reduction_in_recurrent_events * 1.3,
    targeting_mode: "Secondary prevention focus",
  };
}
...
function getTargetedHighImpactIntervention(defaults: Inputs): Partial<Inputs> {
  return {
    ...defaults,
    targeting_mode: "Secondary prevention focus",
    risk_reduction_in_recurrent_events: defaults.risk_reduction_in_recurrent_events * 1.2,
    sustained_engagement_rate: Math.min(defaults.sustained_engagement_rate * 1.08, 1),
    intervention_reach_rate: Math.min(defaults.intervention_reach_rate * 1.05, 1),
  };
}
