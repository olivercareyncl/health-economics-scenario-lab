import { formatCurrency, formatNumber, formatPercent } from "@/lib/frailtyforward/formatters";
import type { AssumptionMeta } from "@/lib/frailtyforward/types";
...
export const ASSUMPTION_META: Record<string, AssumptionMeta> = {
  annual_frailty_cohort_size: {
    ...
    description:
      "Estimated annual number of frail or high-risk patients in scope for the support model.",
  ...
  baseline_crisis_event_rate: {
    ...
    description:
      "Estimated baseline rate of crisis or deterioration events in the cohort.",
  ...
  baseline_non_elective_admission_rate: {
    ...
    formatter: (x) => formatPercent(Number(x)),
    description:
      "Estimated baseline rate of non-elective admissions in the frailty cohort.",
    source_type: "Operational estimate",
    confidence: "Medium confidence",
  },
  current_average_length_of_stay: {
    ...
    description:
      "Average inpatient length of stay for admitted patients in the baseline model.",
    source_type: "Operational estimate",
    confidence: "Medium confidence",
  },
  implementation_reach_rate: {
    ...
    formatter: (x) => formatPercent(Number(x)),
    description:
      "Estimated share of the cohort effectively reached by the community support model.",
    source_type: "Operational estimate",
    confidence: "Medium confidence",
  },
  reduction_in_crisis_event_rate: {
    ...
    formatter: (x) => formatPercent(Number(x)),
    description:
      "Estimated reduction in crisis or deterioration events under the support model.",
  ...
  reduction_in_admission_rate: {
    ...
    formatter: (x) => formatPercent(Number(x)),
    description:
      "Estimated reduction in non-elective admissions under the support model.",
    source_type: "Illustrative default",
    confidence: "Low confidence",
  },
  reduction_in_length_of_stay: {
    ...
    formatter: (x) => formatPercent(Number(x)),
    description:
      "Estimated reduction in average length of stay under the support model.",
    source_type: "Illustrative default",
    confidence: "Low confidence",
  },
  support_cost_per_patient: {
    ...
    description:
      "Estimated programme cost per patient reached by the community support model.",
    source_type: "Operational estimate",
    confidence: "Medium confidence",
  },
  effect_decay_rate: {
  ...
  participation_dropoff_rate: {
    ...
    formatter: (x) => formatPercent(Number(x)),
    description:
      "Estimated annual reduction in effective support reach over time.",
    source_type: "Illustrative default",
    confidence: "Low confidence",
  },
  cost_per_crisis_event: {
    ...
    formatter: (x) => formatCurrency(Number(x)),
    description:
      "Estimated cost associated with a crisis or deterioration event.",
    source_type: "Operational estimate",
    confidence: "Medium confidence",
  },
  cost_per_admission: {
  ...
  costing_method: {
    ...
    description:
      "Method used to value the economic effect of earlier frailty support.",
  ...
  qaly_gain_per_patient_stabilised: {
    ...
    description:
      "Estimated QALY gain for each patient meaningfully stabilised by the support model.",
    source_type: "Evidence-informed proxy",
    confidence: "Low confidence",
  },
  targeting_mode: {
    ...
    formatter: (x) => String(x),
    description:
      "How broadly or narrowly the support model is focused across the frailty cohort.",
    source_type: "User override",
    confidence: "Medium confidence",
  },
  time_horizon_years: {
...
export function getAssumptionConfidenceSummary() {
  const summary: Record<"High confidence" | "Medium confidence" | "Low confidence", number> = {
    "High confidence": 0,
    "Medium confidence": 0,
    "Low confidence": 0,
  };
