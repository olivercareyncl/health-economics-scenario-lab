import { formatCurrency, formatNumber, formatPercent } from "@/lib/pathshift/formatters";
...
export function getAssumptionConfidenceSummary() {
  ...
  Object.values(ASSUMPTION_META).forEach((meta) => {
    const confidence = meta.confidence as keyof typeof summary;
    if (confidence in summary) {
      summary[confidence] += 1;
    }
  });

  summary.summary_text = `${summary["High confidence"]} high-confidence, ${summary["Medium confidence"]} medium-confidence, and ${summary["Low confidence"]} low-confidence assumptions are currently in play.`;
