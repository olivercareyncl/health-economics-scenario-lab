...
def load_defaults() -> dict:
    ...
    fallback_defaults = {
        ...
        "current_acute_managed_rate": 0.40,
        ...
        "reduction_in_follow_up_contacts": 0.20,
        "reduction_in_length_of_stay": 0.10,
        "implementation_reach_rate": 0.70,
...
def build_yearly_results_table(yearly_df: pd.DataFrame) -> pd.DataFrame:
    ...
    display_df = display_df.rename(
        columns={
            "year": "Year",
            "patients_shifted_in_pathway": "Patients shifted in pathway",
            "admissions_avoided": "Admissions avoided",
            "follow_ups_avoided": "Follow-ups avoided",
...
def build_uncertainty_table(uncertainty_df: pd.DataFrame) -> pd.DataFrame:
    ...
    display_df = display_df.rename(
        columns={
            "case": "Case",
            "patients_shifted_total": "Patients shifted in pathway",
    ...
    display_df["Patients shifted in pathway"] = display_df["Patients shifted in pathway"].apply(format_number)
...
def build_scenario_comparison(defaults: dict, base_inputs: dict) -> pd.DataFrame:
    ...
    for scenario_name, scenario_func in SCENARIO_MAP.items():
        ...
        scenario_inputs["cost_per_acute_managed_patient"] = base_inputs["cost_per_acute_managed_patient"]
        scenario_inputs["cost_per_community_managed_patient"] = base_inputs["cost_per_community_managed_patient"]
        scenario_inputs["cost_per_follow_up_contact"] = base_inputs["cost_per_follow_up_contact"]
        ...
        scenario_inputs["qaly_gain_per_patient_improved"] = base_inputs["qaly_gain_per_patient_improved"]
        ...
        rows.append(
            {
                ...
                "Patients shifted in pathway": scenario_results["patients_shifted_total"],
...
def format_scenario_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in ["Patients shifted in pathway", "Admissions avoided"]:
...
def get_best_scenario_text(df: pd.DataFrame) -> dict:
    ...
    best_health_gain = df.loc[df["Patients shifted in pathway"].idxmax()]
...
with st.sidebar:
    ...
    st.header("Cohort and current pathway")
    annual_cohort_size = st.number_input(
        "Annual cohort size",
        min_value=0,
        value=int(scenario_inputs.get("annual_cohort_size", defaults["annual_cohort_size"])),
        step=50,
    )
    current_acute_managed_rate = st.slider(
        "Current acute-managed rate",
        min_value=0.0,
        max_value=1.0,
        value=float(scenario_inputs.get("current_acute_managed_rate", defaults["current_acute_managed_rate"])),
        step=0.01,
    )
    current_admission_rate = st.slider(
        "Current admission rate",
        min_value=0.0,
        max_value=1.0,
        value=float(scenario_inputs.get("current_admission_rate", defaults["current_admission_rate"])),
        step=0.01,
    )
    current_follow_up_contacts_per_patient = st.number_input(
        "Current follow-up contacts per patient",
        min_value=0.0,
        value=float(
            scenario_inputs.get(
                "current_follow_up_contacts_per_patient",
                defaults["current_follow_up_contacts_per_patient"],
            )
        ),
        step=0.1,
    )
    current_average_length_of_stay = st.number_input(
        ...
        value=float(
            scenario_inputs.get(
                ...
                defaults["current_average_length_of_stay"],
            )
        ),
        step=0.5,
    )

    st.header("Redesign assumptions")
    proportion_shifted_to_lower_cost_setting = st.slider(
        "Proportion shifted to lower-cost setting",
        min_value=0.0,
        max_value=0.8,
        value=float(
            scenario_inputs.get(
                "proportion_shifted_to_lower_cost_setting",
                defaults["proportion_shifted_to_lower_cost_setting"],
            )
        ),
        step=0.01,
    )
    ...
    reduction_in_follow_up_contacts = st.slider(
        "Reduction in follow-up contacts",
        min_value=0.0,
        max_value=0.7,
        value=float(
            scenario_inputs.get(
                "reduction_in_follow_up_contacts",
                defaults["reduction_in_follow_up_contacts"],
            )
        ),
        step=0.01,
    )
    ...
    redesign_cost_per_patient = st.number_input(
        "Redesign cost per patient",
        min_value=0.0,
        value=float(
            scenario_inputs.get("redesign_cost_per_patient", defaults["redesign_cost_per_patient"])
        ),
        step=10.0,
    )
    ...
    cost_per_acute_managed_patient = st.number_input(
        "Cost per acute-managed patient",
        min_value=0.0,
        value=float(
            scenario_inputs.get(
                "cost_per_acute_managed_patient",
                defaults["cost_per_acute_managed_patient"],
            )
        ),
        step=50.0,
    )
    cost_per_community_managed_patient = st.number_input(
        "Cost per community-managed patient",
        min_value=0.0,
        value=float(
            scenario_inputs.get(
                "cost_per_community_managed_patient",
                defaults["cost_per_community_managed_patient"],
            )
        ),
        step=50.0,
    )
    cost_per_follow_up_contact = st.number_input(
        "Cost per follow-up contact",
        min_value=0.0,
        value=float(
            scenario_inputs.get(
                "cost_per_follow_up_contact",
                defaults["cost_per_follow_up_contact"],
            )
        ),
        step=10.0,
    )
    ...
    qaly_gain_per_patient_improved = st.number_input(
        "QALY gain per patient meaningfully improved",
        min_value=0.0,
        value=float(
            scenario_inputs.get(
                "qaly_gain_per_patient_improved",
                defaults["qaly_gain_per_patient_improved"],
            )
        ),
        step=0.01,
    )
...
inputs = {
    "annual_cohort_size": annual_cohort_size,
    "current_acute_managed_rate": current_acute_managed_rate,
    "current_admission_rate": current_admission_rate,
    "current_follow_up_contacts_per_patient": current_follow_up_contacts_per_patient,
    "current_average_length_of_stay": current_average_length_of_stay,
    "proportion_shifted_to_lower_cost_setting": proportion_shifted_to_lower_cost_setting,
    "reduction_in_admission_rate": reduction_in_admission_rate,
    "reduction_in_follow_up_contacts": reduction_in_follow_up_contacts,
    "reduction_in_length_of_stay": reduction_in_length_of_stay,
    "implementation_reach_rate": implementation_reach_rate,
    "redesign_cost_per_patient": redesign_cost_per_patient,
    "effect_decay_rate": effect_decay_rate,
    "participation_dropoff_rate": participation_dropoff_rate,
    "cost_per_acute_managed_patient": cost_per_acute_managed_patient,
    "cost_per_community_managed_patient": cost_per_community_managed_patient,
    "cost_per_follow_up_contact": cost_per_follow_up_contact,
    "cost_per_admission": cost_per_admission,
    "cost_per_bed_day": cost_per_bed_day,
    "costing_method": costing_method,
    "qaly_gain_per_patient_improved": qaly_gain_per_patient_improved,
...
with tab4:
    ...
    st.write(
        "These preset scenarios illustrate how different redesign strategies change activity and value over the selected horizon."
    )
    ...
    with summary_col3:
        st.metric(
            ...
            format_number(best_scenarios["best_health_gain"]["Patients shifted in pathway"]) + " shifted",
        )
...
st.markdown("---")
st.caption(
    "PathShift is part of the Health Economics Scenario Lab — a series of interactive decision sandboxes for exploring value under uncertainty."
)