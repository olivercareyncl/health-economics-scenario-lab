...
def make_scenario_outcome_chart(scenario_df):
    ...
    fig.add_trace(
        go.Bar(
            x=scenario_df["Scenario"],
            y=scenario_df["Patients shifted in pathway"],
            name="Patients shifted in pathway",
            text=[f"{v:,.0f}" for v in scenario_df["Patients shifted in pathway"]],
            textposition="outside",
        )
    )
...
def make_pathway_shift_chart(yearly_df):
    fig = go.Figure(
        data=[
            go.Bar(
                x=yearly_df["year"],
                y=yearly_df["patients_shifted_in_pathway"],
                text=[f"{v:,.0f}" for v in yearly_df["patients_shifted_in_pathway"]],
                textposition="outside",
                showlegend=False,
            )
        ]
    ...
    fig.update_layout(
        title="Patients shifted in pathway by year",
        xaxis_title="Year",
        yaxis_title="Patients shifted",
        height=430,
        margin=dict(l=20, r=20, t=80, b=20),
        showlegend=False,
    )
