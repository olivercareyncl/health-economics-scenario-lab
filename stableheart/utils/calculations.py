...
def get_targeting_adjustments(inputs: dict) -> dict:
    ...
    adjusted_population = inputs["eligible_population"] * targeting["population_multiplier"]
    adjusted_reach_rate = clamp_rate(inputs["intervention_reach_rate"] * targeting["reach_multiplier"])
    adjusted_event_rate = clamp_rate(inputs["baseline_recurrent_event_rate"] * targeting["risk_multiplier"])

    return {
        "adjusted_population": adjusted_population,
        "adjusted_reach_rate": adjusted_reach_rate,
        "adjusted_event_rate": adjusted_event_rate,
    }
...
def calculate_gross_savings(
    ...
    event_and_admission_savings = (
        events_avoided * inputs["cost_per_cardiovascular_event"]
        + admissions_avoided * inputs["cost_per_admission"]
    )
    bed_day_value = bed_days_avoided * inputs["cost_per_bed_day"]

    if costing["mode"] == "event_admission":
        return event_and_admission_savings
    if costing["mode"] == "bed_day":
        return bed_day_value
    return event_and_admission_savings + bed_day_value


def _run_model_core(inputs: dict) -> dict:
    targeting = get_targeting_adjustments(inputs)
    base_reached_patients = targeting["adjusted_population"] * targeting["adjusted_reach_rate"]
    ...
    for year in range(1, inputs["time_horizon_years"] + 1):
        effect_multiplier = (1 - inputs["annual_effect_decay_rate"]) ** (year - 1)
        participation_multiplier = (1 - inputs["annual_participation_dropoff_rate"]) ** (year - 1)

        patients_reached = base_reached_patients * participation_multiplier
        effective_patients = patients_reached * inputs["sustained_engagement_rate"]

        event_risk_reduction = clamp_rate(inputs["risk_reduction_in_recurrent_events"] * effect_multiplier)

        baseline_events = effective_patients * targeting["adjusted_event_rate"]
        events_avoided = baseline_events * event_risk_reduction

        admissions_avoided = events_avoided * inputs["admission_probability_per_event"]
        ...
        qalys_gained = events_avoided * inputs["qaly_gain_per_event_avoided"]
    ...
    patients_reached_total = yearly_results["patients_reached"].sum()
    events_avoided_total = yearly_results["events_avoided"].sum()
    ...
    return {
        ...
        "events_avoided_total": events_avoided_total,
...
def run_model(inputs: dict) -> dict:
    ...
    core.update(
        {
            "break_even_risk_reduction_required": clamp_rate(calculate_break_even_risk_reduction(inputs)),
            ...
            "break_even_baseline_event_rate_required": clamp_rate(calculate_break_even_baseline_event_rate(inputs)),
            "break_even_qaly_gain_required": calculate_break_even_qaly_gain(inputs),
        }
    )
    return core


def calculate_break_even_risk_reduction(inputs: dict) -> float:
    targeting = get_targeting_adjustments(inputs)
    base_reached_patients = targeting["adjusted_population"] * targeting["adjusted_reach_rate"]
    ...
    for year in range(1, inputs["time_horizon_years"] + 1):
        ...
        events_per_unit = effective_patients * targeting["adjusted_event_rate"] * effect_multiplier
        admissions_per_unit = events_per_unit * inputs["admission_probability_per_event"]
        bed_days_per_unit = admissions_per_unit * inputs["average_length_of_stay"]
...
def calculate_break_even_baseline_event_rate(inputs: dict) -> float:
    targeting = get_targeting_adjustments(inputs)
    base_reached_patients = targeting["adjusted_population"] * targeting["adjusted_reach_rate"]
    ...
    for year in range(1, inputs["time_horizon_years"] + 1):
        ...
        events_per_unit_rate = effective_patients * inputs["risk_reduction_in_recurrent_events"] * effect_multiplier
        admissions_per_unit_rate = events_per_unit_rate * inputs["admission_probability_per_event"]
        bed_days_per_unit_rate = admissions_per_unit_rate * inputs["average_length_of_stay"]

        gross_savings_per_unit_rate = calculate_gross_savings(
            events_per_unit_rate,
            admissions_per_unit_rate,
            bed_days_per_unit_rate,
            inputs,
        )
        qaly_value_per_unit_rate = (
            events_per_unit_rate
            * inputs["qaly_gain_per_event_avoided"]
            * inputs["cost_effectiveness_threshold"]
        )
        ...
        discount_factor = get_discount_factor(year, inputs["discount_rate"])
        numerator += patients_reached * inputs["intervention_cost_per_patient_reached"] * discount_factor
        denominator += (gross_savings_per_unit_rate + qaly_value_per_unit_rate) * discount_factor

    return safe_divide(numerator, denominator)


def calculate_break_even_qaly_gain(inputs: dict) -> float:
    result = _run_model_core(inputs)
    if result["events_avoided_total"] <= 0:
        return 0.0

    required_total_qaly_value = max(
        result["discounted_net_cost_total"] / inputs["cost_effectiveness_threshold"],
        0,
    )
    return safe_divide(required_total_qaly_value, result["events_avoided_total"])
...
def run_bounded_uncertainty(inputs: dict) -> pd.DataFrame:
    cases = {
        "Low": {
            "risk_reduction_in_recurrent_events": clamp_rate(inputs["risk_reduction_in_recurrent_events"] * 0.8),
            "intervention_cost_per_patient_reached": inputs["intervention_cost_per_patient_reached"] * 1.2,
            "baseline_recurrent_event_rate": clamp_rate(inputs["baseline_recurrent_event_rate"] * 0.85),
            "qaly_gain_per_event_avoided": inputs["qaly_gain_per_event_avoided"] * 0.8,
            "sustained_engagement_rate": clamp_rate(inputs["sustained_engagement_rate"] * 0.9),
        ...
        "High": {
            "risk_reduction_in_recurrent_events": clamp_rate(inputs["risk_reduction_in_recurrent_events"] * 1.2),
            "intervention_cost_per_patient_reached": inputs["intervention_cost_per_patient_reached"] * 0.8,
            "baseline_recurrent_event_rate": clamp_rate(inputs["baseline_recurrent_event_rate"] * 1.15),
            "qaly_gain_per_event_avoided": inputs["qaly_gain_per_event_avoided"] * 1.2,
            "sustained_engagement_rate": clamp_rate(inputs["sustained_engagement_rate"] * 1.05),
...
def build_comparator_case(defaults: dict, base_inputs: dict, comparator_mode: str) -> dict:
    ...
    comparator_inputs["cost_per_cardiovascular_event"] = base_inputs["cost_per_cardiovascular_event"]
    ...
    comparator_inputs["qaly_gain_per_event_avoided"] = base_inputs["qaly_gain_per_event_avoided"]
