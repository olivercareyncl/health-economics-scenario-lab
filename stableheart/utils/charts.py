...
def make_events_chart(results: dict):
    categories = [
        "Events avoided",
        "Admissions avoided",
        "Bed days avoided",
    ...
    fig.update_layout(
        title="Clinical impact over selected horizon",
        xaxis_title="",
...
def make_event_cascade_chart(results: dict):
    fig = go.Figure(
        go.Waterfall(
            name="Event cascade",
            orientation="v",
            ...
            y=[
                ...
                results["admissions_avoided_total"],
                results["bed_days_avoided_total"],
                results["discounted_qalys_total"],
            ],
            text=[
                f"{results['events_avoided_total']:,.0f}",
                f"{results['admissions_avoided_total']:,.0f}",
                f"{results['bed_days_avoided_total']:,.0f}",
                f"{results['discounted_qalys_total']:,.2f}",
            ],
            textposition="outside",
        )
    )
    fig.update_layout(
        title="Value cascade from recurrent events avoided",
        xaxis_title="",
        yaxis_title="Impact",
        showlegend=False,
        height=430,
...
def make_scenario_outcome_chart(scenario_df):
    ...
    fig.add_trace(
        go.Bar(
            x=scenario_df["Scenario"],
            y=scenario_df["Events avoided"],
            name="Events avoided",
            text=[f"{v:,.0f}" for v in scenario_df["Events avoided"]],
            textposition="outside",
        )
    )
    ...
    fig.update_layout(
        title="Headline impact across scenarios",
        barmode="group",
        xaxis_title="",
        yaxis_title="Estimated volume",
        height=450,
