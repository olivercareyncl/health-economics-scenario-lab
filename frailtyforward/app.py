...
st.subheader("Frailty Support Sandbox")
st.write(
    "Explore how earlier frailty support might change crisis events, admissions, bed use, and value under different assumptions about reach, effectiveness, persistence, and delivery cost."
)

st.info(
    "Key question: What would need to be true for earlier frailty support to create value?"
)
...
with tab1:
    ...
    primary_1.metric("Patients stabilised", format_number(results["patients_stabilised_total"]))
    primary_2.metric("Crisis events avoided", format_number(results["crisis_events_avoided_total"]))
    primary_3.metric(
        net_cost_label,
    ...
    if decision_status == "Appears cost-saving":
        ...
        st.caption(
            "The current assumptions suggest the support model generates discounted net savings over the selected horizon."
        )
    elif decision_status == "Appears cost-effective":
        st.info("Appears cost-effective")
        st.caption(
            "The current assumptions suggest the support model is below the selected cost-effectiveness threshold, but not cost-saving."
        )
    else:
        st.warning("Above threshold")
        st.caption(
            "The current assumptions suggest the support model delivers benefit, but remains above the selected threshold."
        )
    ...
    st.plotly_chart(
        make_stabilised_patients_chart(results["yearly_results"]),
