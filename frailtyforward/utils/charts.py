...
def make_scenario_outcome_chart(scenario_df):
    ...
    fig.add_trace(
        go.Bar(
            x=scenario_df["Scenario"],
            y=scenario_df["Patients stabilised"],
            name="Patients stabilised",
            text=[f"{v:,.0f}" for v in scenario_df["Patients stabilised"]],
            textposition="outside",
        )
    )
...
def make_patients_stabilised_chart(yearly_df):
    fig = go.Figure(
        data=[
            go.Bar(
                x=yearly_df["year"],
                y=yearly_df["patients_stabilised"],
                text=[f"{v:,.0f}" for v in yearly_df["patients_stabilised"]],
                textposition="outside",
                showlegend=False,
            )
        ]
    ...
    fig.update_layout(
        title="Patients stabilised by year",
        xaxis_title="Year",
        yaxis_title="Patients stabilised",
        height=430,
        margin=dict(l=20, r=20, t=80, b=20),
        showlegend=False,
    )
